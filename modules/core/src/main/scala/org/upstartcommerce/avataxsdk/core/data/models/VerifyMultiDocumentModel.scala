...
final case class VerifyMultiDocumentModel(code: String, `type`: DocumentType, verifyTransactionDate: Option[Instant] = None, verifyTotalAmount: Option[BigDecimal] = None, verifyTotalTax: Option[BigDecimal] = None) {
  ...
  def withVerifyTransactionDate(value: Instant): VerifyMultiDocumentModel = copy(verifyTransactionDate = Some(value))
