...
final case class TransactionReferenceFieldModel(documentId: Long, reportingLocationCode: Option[String] = None, lineDetailSerCodes: Option[List[LineDetailSERCodeModel]] = None) {
  lazy val lineDetailSerCodesRaw: List[LineDetailSERCodeModel] = lineDetailSerCodes.getOrElse(List.empty)

  def withDocumentId(value: Long): TransactionReferenceFieldModel = copy(documentId = value)
